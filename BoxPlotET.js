var bpetchartWidth = 550;
var bpetchartHeight = 300;
var bpetmargin = { top: 40, right:20, bottom: 0, left: 70};

var boxplot1 = d3.select("#BoxPlotET")
    .style("width", bpetchartWidth + "px")
    .style("height", bpetchartHeight + "px");

var bpet = boxplot1.append("svg")
    .attr("width", bpetchartWidth)
    .attr("height", bpetchartHeight)
    .append("g")
    .attr("transform", "translate(" + bpetmargin.left + "," + bpetmargin.top + ")");
...
d3.csv("ds_salaries.csv").then(function (dataset) {
    ...
    function createBoxPlotData(workYear) {
        ...
        const labeledEmploymentType = employmentTypeLabels[key];
        if (!acc[labeledEmploymentType]) {
            acc[labeledEmploymentType] = [];
        }
        acc[labeledEmploymentType].push(d.salary_in_usd);
        return acc;
        }, {});
    ...
    function createBoxPlots(workYear) {
        ...
        // Remove old elements
        bpet.selectAll(".box, .whisker, .median-line, .dot, .min-line, .max-line, .axis-label").remove();
        bpet.select(".x-axis").remove();
        bpet.select(".y-axis").remove();
        bpet.select(".chart-title").remove();

        // Scales
        const xScale = d3.scaleBand()
        .domain(["Part Time", "Full Time", "Contract", "Freelance"])
        .range([0, width])
        .padding(0.1);
        ...
        .join(
            ...
            .attr("class", "box")
            .attr("x", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth() * 0.25;
            })
            .attr("y", function (d) {
                return yScale(d.q3);
            })
        ...
        .join(
            ...
            .attr("class", "whisker")
            .attr("x1", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth() * 0.5;
            })
            .attr("x2", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth() * 0.5;
            })
        ...
        .join(
            ...
            .attr("x1", function (d) {
                return xScale(d.employmentType);
            })
            .attr("x2", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth();
            })
            .attr("y1", function (d) {
        ...
        .join(
            ...
            .join("circle")
            .attr("cx", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth() / 2;
            })
            .attr("cy", function (d) {
        ...
        .join(
            ...
            .attr("class", "min-line")
            .attr("x1", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth() / 4; // Adjust the x position to align with the whisker
            })
            .attr("y1", function (d) {
                return yScale(d.lowerWhisker);
            })
            .attr("x2", function (d) {
                return xScale(d.employmentType) + xScale.bandwidth() * 3 / 4; // Adjust the x position to align with the whisker
            })
            .attr("y2", function (d) {
                return yScale(d.lowerWhisker);
            })
        ...
        // Create tooltip
        const tooltipET = d3.select("#tooltipET");

        // Add event listeners to show/hide tooltips
        bpet.selectAll(".box, .dot, .min-label, .max-label")
        .on("mouseover", function (event, d) {
            // Calculate tooltip position
            const xPosition = xScale(d.employmentType) + 800;
            const yPosition = yScale(d.median) +600;

            // Build tooltip HTML content
            const tooltipContent = `
            <strong>Employment Type:</strong> ${d.employmentType}<br>
