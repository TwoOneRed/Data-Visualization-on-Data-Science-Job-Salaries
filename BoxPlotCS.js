var bpcschartWidth = 600;
var bpcschartHeight = 300;
var bpcsmargin = { top: 40, right:20, bottom: 0, left: 70};

var boxplot1 = d3.select("#BoxPlotCS")
    .style("width", bpcschartWidth + "px")
    .style("height", bpcschartHeight + "px");

var bpcs = boxplot1.append("svg")
    .attr("width", bpcschartWidth)
    .attr("height", bpcschartHeight)
    .append("g")
    .attr("transform", "translate(" + bpcsmargin.left + "," + bpcsmargin.top + ")");
...
dataset.forEach(function(d) {
  ...
  function createBoxPlots(workYear) {
    ...
    // Remove old elements
    bpcs.selectAll(".box, .whisker, .median-line, .dot, .min-line, .max-line, .axis-label").remove();
    bpcs.select(".x-axis").remove();
    bpcs.select(".y-axis").remove();
    bpcs.select(".chart-title").remove();
    ...
    const W = d3.max(boxplotData, function(d) {
    ...
    // Create tooltip-CS
    const tooltipCS = d3.select("#tooltipCS");

    // Add event listeners to show/hide tooltips
    bpcs.selectAll(".box, .dot, .min-label, .max-label")
